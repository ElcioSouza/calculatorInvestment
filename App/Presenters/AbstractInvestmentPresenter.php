...
abstract class AbstractInvestmentPresenter
{
    ...
    protected function renderInvestmentDetails(
        ...
        printf($mNormal, 'Tipo da taxa:', strtoupper($rateType));
        ...
        printf($mAcento, 'Data de Aplicação:', date('d/m/Y', strtotime($input->applicationDate)));
        ...
        printf($mNormal, 'Dias Úteis médios por mês (Base 252):', (string) $businessDaysPerMonth);
    ...
    protected function renderGrossSection(Investment $result, callable $money): void
    {
        ...
        printf($mNormal, 'Lucro Bruto no período:', $money($result->profitBruto));
        printf($mNormal, 'Lucro Bruto por dia:', $money(bcdiv($result->profitBruto, (string) $result->days, 2)));
        printf($mNormal, 'Lucro Bruto por dia útil (1º dia):', $money($result->dailyProfitDisplay));
    ...
    protected function renderNetSection(Investment $result, callable $money): void
    {
        ...
        printf($mNormal, 'Lucro Líquido por mês:', $money($result->monthlyProfitLiquid));
        printf(
            $mNormal,
    ...
    protected function resolveTaxaSelicAtual(InvestmentInput $input): string
    {
        return $input->cdiOver !== ''
            ? $input->cdiOver
            : number_format(((float) $input->selicMeta) - 0.10, 8, '.', '');
    }
