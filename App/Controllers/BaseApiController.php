    ...
    protected function buildPayload(InvestmentInput $input, Investment $result): array
    {
        return [
            'input'  => [
                ...
                'selic_meta'       => $input->selicMeta !== '' ? (float) $input->selicMeta : ($input->selicMetaDefault !== '' ? (float) $input->selicMetaDefault : null),
            ...
            'result' => [
                ...
                'profit_percentage'     => (float) number_format((float) $result->profitPercentage, 2, '.', ''),
            ],
        ];
    }
}
